...
1. <font color='orange'>更简便的NoSQL支持</font>
...
9. <font color='orange'>字符集支持</font>

   MySQL 8中默认的字符集由 latin1 更改为 utf8mb4 ，并首次增加了日语所特定使用的集 合，utf8mb4_ja_0900_as_cs。

10. <font color='orange'>优化器增强</font>

    MySQL优化器开始支持隐藏索引和降序索引。隐藏索引不会被优化器使用，验证索引的必 要性时不需要删除索引，先将索引隐藏，如果优化器性能无影响就可以真正地删除索引。降序索引允许 优化器对多个列进行排序，并且允许排序顺序不一致。

11. <font color='orange'>公用表表达式</font> 

    公用表达式（Common Table Expressions）简称为CTE，MySQL现在支持递归和非递归两种形式的CTE。CTE通过在SELECT语句或其它特定语句前==使用WITH语句对临时结果集==进行命名
...
12. 窗口函数
    ...
    MySQL 8开始支持窗口函数。在之前的版本中已存在的大部分==聚合函数==在M有SQL 8中也可以作为窗口函数来使用。 （待处理）
...
13. <font color='orange'>正则表达式支持</font>
...
14. <font color='orange'>内部临时表</font>

==TempTable存储引擎取代MEMORY存储引擎成为内部临时表的默认存储引擎==。TempTable存储引擎为VARCHAR和VARBINARY列提供高效存储。internal_tmp_mem_storage_engine会话变量定义了内部 临时表的存储引擎，可选的值有两个，TempTable和MEMORY，其中TempTable为默认的存储引擎。 temptable_max_ram系统配置项定义了TempTable存储引擎可使用的最大内存数量。

15. <font color='orange'>日志记录</font>

在MySQL 8中错误日志子系统由一系列MySQL组件构成。这些组件的构成由系统变量 log_error_services来配置，能够实现日志事件的过滤和写入。 待处理
...
* 文字描述：
  * 请求处理流程主要分为三部分：**连接管理**、**解析与优化**、**存储引擎**
  * 解析与优化阶段有分为：**查询缓存**、**语法解析**、**查询优化**

![image-20240304100640952](MySql.assets\image-20240304100640952.png)
...
![image-20240304111223939](MySql.assets\image-20240304111223939.png)
...
> ### 语法树解析
>
> 抽象语法树（Abstract Syntax Tree，AST）和具体语法树（Concrete Syntax Tree，CST）是在编译器和解释器中常用的数据结构，用于表示源代码的结构和语义信息。
>
> 1. 具体语法树（CST）：
>    具体语法树是从源代码逐字符解析生成的树状结构。它反映了源代码的确切语法结构，包括了所有的细节和符号。具体语法树通常使用自上而下递归地解析源代码，构建树状结构。CST 可以视为源代码的一个直接映射，每个节点都对应着源代码的每个符号或语法结构。
> 2. 抽象语法树（AST）：
>    抽象语法树是在具体语法树的基础上，去除了冗余的细节和符号，只保留语法结构的核心信息的树状结构。AST 包含了源代码的抽象概念，用于表示程序的结构和语义。AST 中的每个节点代表着源代码的一个关键结构，比如声明、表达式、控制流语句等。AST 在编译器和解释器中被广泛应用，用于静态分析、优化、中间代码生成等。
>
> 在生成抽象语法树的过程中，通常包含以下步骤：
>
> - 词法分析：将源代码分割成一个个的标识符和符号，形成词法单元（Token）。
> - 语法分析：根据语法规则，将词法单元组织成具体语法树（CST）。
> - 语义分析：对具体语法树进行进一步的处理和转换，去除冗余信息，生成抽象语法树（AST）。
>
> AST 是一种语义化的数据结构，在编译器前端和后端的各个阶段都可以使用，可以进行语法检查、类型检查、代码生成等操作。

## 2、SQL执行流程

### 2.1 MySQL中的SQL执行流程

![image-20240304134052366](MySql.assets\image-20240304134052366.png)
...
* MySQL的查询流程：
  ...
  1. **查询缓存**：Mysql8没有了
     ...
     * 原因：
       * 缓存效率取决于命中率。mysql不缓存查询计划，只缓存查询结果，相当于相同sql语句但不同的查询条件会导致大量的缓存结果。
       * 用户自定义变量和函数、一些系统表，如 mysql 、 information_schema、 performance_schema 数据库中的表，那这个请求就不会被缓存。比如：**NOW**
       * 既然是缓存，那就有它 缓存失效的时候，对于 更新压力大的数据库 来说，查询缓存的命中率会非常低。
  ...
  2. **解析器**
     ...
     * 在解析器中对 SQL 语句进行语法分析、语义分析。
       ...
       2. ==语法分析==
          ...
          * ![image-20240304135612035](MySql.assets\image-20240304135612035.png)

  3. **优化器**：

     * 在优化器中会确定 SQL 语句的**执行路径（执行顺序）**，比如是根据 **全表检索** ，还是根据 **索引检索** 等。
  ...
  5. **总结**：
     ...
     * ![image-20240304142509341](MySql.assets\image-20240304142509341.png)

       

### 2.2、MySQL8中SQL性能分析

> #### MySQL性能分析神器：explain、profiling

1. **开启profiling (会话和全局变量)**
...
3. **查看profiles**
   ...
   * ```mysql
     ...
     +----------+------------+-------------------------------+
     |        3 | 0.00032275 | select @@global.profiling     |
     |        4 | 0.00024125 | select @@session.profiling    |
     |        5 | 0.00010025 | select * from  user           |
     |        6 | 0.00012600 | select * from user            |
     |        7 | 0.00021975 | SELECT DATABASE()             |
     |        8 | 0.00012400 | seelct * from user            |
     |        9 | 0.00492300 | select * from user            |
     |       10 | 0.00013700 | select * from user where =2   |
     |       11 | 0.00055300 | select * from user where id=2 |
     |       12 | 0.00039600 | select * from user where id=2 |
     |       13 | 0.00030275 | select * from user where id=2 |
     |       14 | 0.00031575 | select * from user where id=2 |
     |       15 | 0.00039650 | select * from user where id=2 |
     |       16 | 0.00029450 | select * from user where id=2 |
     |       17 | 0.00039150 | select * from user where id=2 |
     +----------+------------+-------------------------------+
     ```
...
4. **查看profile**
   ...
   * 显示执行计划，查看程序的执行步骤
     ...
     * ```mysql
       ...
       | Opening tables                 | 0.000038 |打开表
       | init                           | 0.000003 |初始化
       | System lock                    | 0.000006 |锁系统
       | optimizing                     | 0.000009 |优化查询
       | statistics                     | 0.000048 |
       | preparing                      | 0.000012 |准备
       | executing                      | 0.000009 |执行
       | end                            | 0.000054 |
       | query end                      | 0.000002 |
   ...
   * 此外，还可以查询更丰富的内容：
     ...
     * ```mysql
       ...
       +--------------------------------+----------+----------+------------+--------------+---------------+
       | starting                       | 0.000071 | 0.000000 |   0.000000 |         NULL |
          NULL |
       | Executing hook on transaction  | 0.000003 | 0.000000 |   0.000000 |         NULL |          NULL |
       | starting                       | 0.000007 | 0.000000 |   0.000000 |         NULL |          NULL |
       | checking permissions           | 0.000005 | 0.000000 |   0.000000 |         NULL |
          NULL |
       ...
       | optimizing                     | 0.000010 | 0.000000 |   0.000000 |         NULL |          NULL |
       | statistics                     | 0.000048 | 0.000000 |   0.000000 |         NULL |          NULL |
...
### 2.3 MySQL5.7中SQL性能分析

上述操作在**MySQL5.7**中测试，发现前后两次相同的sql语句，执行的查询过程仍然是相同的。不是会使用 缓存吗？这里我们需要 **显式开启查询缓存模式** 。在MySQL5.7中如下设置：

在 /etc/my.cnf 中新增一行： query_cache_type=1  重启mysql服务 ，其它一样
...
在**InnoDB**存储引擎中有一部分数据会放到内存中，缓冲池则占了这部分内存的大部分，它用来存储各种数据的缓存。如下：
...
其次，频次决定有限级顺序，因为缓冲池的大小是有限的，比如磁盘有200G，但是内存只有16G，缓冲池大小只有1G，就无法将所有数据都加载到缓冲池里，这时就涉及到优先级顺序，会优**先对使用频次高的热数据进行加载**。
...
![image-20240304171316176](MySql.assets\image-20240304171316176.png)
...
### 3.4 多个Buffer Poll 实例

查看 `innodb_buffer_pool_instances`
...
![image-20240304172541342](MySql.assets\image-20240304172541342.png)

我更新到一半突然发生错误了，想要回滚到更新之前的版本，该怎么办？连数据持久化的保证、事务回 滚都做不到还谈什么**崩溃恢复**？
